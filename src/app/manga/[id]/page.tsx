...
import DisplayInformation from "@/app/components/DisplayInformation/DisplayInformation";
import {
...
const Page = async ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const mangaData = await getAnimeResponseObject({
    resource: `manga/${id}/full`,
  });

  const characterData = await getDataResponse({
    resource: `manga/${id}/characters`,
    query: `limit=10`,
  });
  ...
  return (
    ...
    //       </div>
    //       <div className="mb-2">
    //         <p className="font-semibold text-2xl mb-1">Characters</p>
    //         <hr className="w-full border-white/40 border-solid border-[1px] rounded mb-2" />
